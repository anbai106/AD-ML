...
class DWIRegionInput(CAPSInput):
    ...
    def get_images(self):
        ...
        self._images = [path.join(self._caps_directory, 'subjects', self._subjects[i], self._sessions[i],
                                      'dwi/dti_based_processing/atlas_statistics', '%s_%s_acq-axial_dwi_space-%s_res-1x1x1_map-%s_statistics.tsv'
                                      % (self._subjects[i], self._sessions[i], self._atlas, (self._dwi_map).upper()))
