...
class RepeatedHoldOutFeautureSelection(base.MLValidation):
    ...
    def validate(self, y, n_threads=15, splits_indices=None, inner_cv=True, top_k=50):
        ...
        for i in range(self._n_iterations):
            ...
            train_index, test_index = self._cv[i]
            if inner_cv:
                async_result[i] = async_pool.apply_async(self._ml_algorithm.evaluate, (train_index, test_index, top_k))
            else:
                async_result[i] = async_pool.apply_async(self._ml_algorithm.evaluate_no_cv, (train_index, test_index))
